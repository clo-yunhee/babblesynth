...
{
public:
    using value_type = std::variant<std::monostate, int, double, bool, std::string, enumeration_value, variable_plan>;
    using observer_type = std::function<void(const parameter&)>;
    ...
    bool isRanged() const
    {
        return m_min.index() != 0 || m_max.index() != 0;
    }
    ...
    template<typename T>
    std::enable_if_t<util::is_any_v<T, int, double>, parameter&> setMin(const T& min)
    {
        m_min = min;
        return *this;
    }
    ...
    template<typename T>
    std::enable_if_t<util::is_any_v<T, int, double>, parameter&> setMax(const T& max)
    {
        m_max = max;
        return *this;
    }
