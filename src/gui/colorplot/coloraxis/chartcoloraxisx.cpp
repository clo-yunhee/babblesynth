...
ChartColorAxisX::ChartColorAxisX(QColorAxis *axis, QGraphicsItem *item)
    : HorizontalAxis(axis, item, true), m_axis(axis) {}

ChartColorAxisX::~ChartColorAxisX() {}

QSizeF ChartColorAxisX::sizeHint(Qt::SizeHint which,
    ...
    switch (which) {
        case Qt::MinimumSize: {
            ...
            height = boundingRect.height() + labelPadding() + base.height() +
        ...
        case Qt::PreferredSize: {
            ...
            height = labelHeight + labelPadding() + base.height() +
                     m_axis->size() + colorScalePadding() + 1.0;
...
QVector<qreal> ChartColorAxisX::calculateLayout() const {
