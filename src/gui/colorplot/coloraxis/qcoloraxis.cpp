...
ACCESS_PRIVATE_FIELD(QtCharts::QAbstractAxisPrivate,
...
/*!
    \class QColorAxis
    \inmodule QtCharts
    \brief The QColorAxis class displays a color scale as one of the
   chart's axes. \since 6.2

    A color axis can be set up to show a color scale based on the passed
   gradient. The scale has tick marks with labels based on data passed
   in QXYSeries::colorby method.
*/
...
/*!
  ...
  When setting this property, the maximum value is adjusted if
...
/*!
  ...
  When setting this property, the minimum value is adjusted if
  necessary, to ensure that the range remains valid.
*/

/*!
  \property QColorAxis::tickCount
  \brief The number of tick marks on the axis. This indicates how many
  grid lines are drawn on the chart if QColorAxis::gridVisible is equal
  to \c true. The default value is 5, and the number cannot be less
  than 2.

  \note Grid lines are intentionally invisible by default in QColorAxis
...
/*!
  ...
  \brief The property indicating if the range should be set from the
  list of values passed in QXYSeries::colorBy method or rather taken
...
/*!
  ...
  This signal is emitted when the minimum value of the axis, specified
  by \a min, changes.
*/

/*!
  \fn void QColorAxis::maxChanged(qreal max)
  This signal is emitted when the maximum value of the axis, specified
  by \a max, changes.
*/
...
/*!
  ...
  specified by \a tickCount, changes.
*/

/*!
  \fn void QColorAxis::sizeChanged(qreal size)
  This signal is emitted when the size of the color scale, specified by
  \a size, changes.
*/

/*!
  \fn void QColorAxis::autoRangeChanged(bool autoRange)
  This signal is emitted when the auto range mode, specified by \a
  autoRange, changes.
*/
...
/*!
  ...
  specified by \a min and \a max, changes.
*/
...
QAbstractAxis::AxisType QColorAxis::type() const { return AxisTypeColor; }
...
void QColorAxisPrivate::setRange(qreal min, qreal max) {
    ...
    if (!isValidValue(min, max)) {
        qWarning() << "Attempting to set invalid range for value axis: [" << min
                   << " - " << max << "]";
        return;
    }
...
void QColorAxisPrivate::updateSeries() {
    ...
    for (const auto &serie : series) {
        if (serie->type() == QColorXYSeries::SeriesTypeColorLine) {
            QColorXYSeries *xySeries = static_cast<QColorXYSeries *>(serie);
            const auto &colorByData = xySeries->d_func()->colorByData();
            if (!colorByData.isEmpty()) xySeries->colorBy(colorByData);
        }
    }
}
